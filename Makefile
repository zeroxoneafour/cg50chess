...
#---------------------------------------------------------------------------------
ifeq ($(strip $(FXCGSDK)),)
export FXCGSDK := $(abspath src/libfxcg/)
endif
...
MKG3AFLAGS := -n basic:Chess -i uns:../res/unselected.bmp -i sel:../res/selected.bmp
...
# install to calculator
install: $(OUTPUT).g3a
	cp -f $(OUTPUT).g3a "/run/media/$(USER)/disk/"
